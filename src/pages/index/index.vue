...
function updateConfig(){
  setCookie('config', JSON.stringify({
    soundEffectVolume: gameGm.soundEffectVolume,
    bgmVolume: gameGm.bgmVolume
  }), 90)
}
...
function readConfig(){
  ...
  if(cookieConfig != null && cookieConfig != ''){
    ...
    if(config.bgmVolume){
      gameGm.bgmVolume = config.bgmVolume
    }
  }
}
...
<template>
  <div id="tiger-atm">
    ...
    <header>
      <div>
        <span style="font-weight: 600;">奖金数:</span>
        <Scoreboard :length="4" :value="fraction"  />
      </div>
      <div>
        <span style="font-weight: 600;">金币数:</span>
        <Scoreboard :length="4" :value="goldCoin" />
      </div>
    </header>
    ...
    <Transition>
      <div v-if="dialogs.setting.visible" class="setting">
        <ul>
          ...
          <li>
            <b>音效音量</b><input type="range" v-model="gameGm.soundEffectVolume"
              :volume="gameGm.soundEffectVolume / 100" />
          </li>
          <li>
